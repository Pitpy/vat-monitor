<template>
  <div class="container">
    ...
    <table>
      <thead>
        <tr>
          <th scope="col" class="l">{{header.report.type}}</th>
          <th scope="col" class="r">{{header.report.cntVehicle}}</th>
          <th scope="col" class="r">{{header.report.cntLao}}</th>
          <th scope="col" class="r">{{header.report.cntForeigner}}</th>
          <th scope="col" class="r">{{header.report.cntBorderPass}}</th>
          <th scope="col" class="r">{{header.report.cntPassport}}</th>
          <th scope="col" class="r">{{header.report.fees}}</th>
          <th scope="col" class="r">{{header.report.otFees}}</th>
        </tr>
      </thead>

      <tbody>
        <tr v-for="json_data in json_datas" :key="json_data.index">
          <td scope="row" class="l" :data-label="header.report.type">{{json_data.type}}</td>
          <td
            class="tdd r"
            :data-label="header.report.cntVehicle"
          >{{ num(json_data.cntVehicle).format('0,0') }}</td>
          <td
            class="tdd r"
            :data-label="header.report.cntLao"
          >{{ num(json_data.cntLao).format('0,0') }}</td>
          <td
            class="tdd r"
            :data-label="header.report.cntForeigner"
          >{{ num(json_data.cntForeigner).format('0,0') }}</td>
          <td
            class="tdd r"
            :data-label="header.report.cntBorderPass"
          >{{ num(json_data.cntBorderPass).format('0,0') }}</td>
          <td
            class="tdd r"
            :data-label="header.report.cntPassport"
          >{{ num(json_data.cntPassport).format('0,0') }}</td>
          <td class="tdd r" :data-label="header.report.fees">{{ num(json_data.fees).format('0,0') }}</td>
          <td
            class="tdd r"
            :data-label="header.report.otFees"
          >{{ num(json_data.otFees).format('0,0') }}</td>
        </tr>
      </tbody>
    </table>
  </div>
...
export default {
  ...
  methods: {
    ...
    search() {
      this.getData("passenger", this.rpOpt, this.startDate, this.endDate);
    },
    exportDoc(e) {
      if (e == 1) {
        location.href = location.href.replace(
          ...
          `http://${
            location.host
          }/report/excel_passenger.php?type=passenger&option=${
            this.rpOpt
        ...
        location.href = location.href.replace(
          ...
          `http://${
            location.host
          }/report/pdf_passenger.php?type=passenger&option=${
            this.rpOpt
          }&from=${moment(this.startDate).format("YYYY-MM-DD")}&to=${moment(
            this.endDate
          ).format("YYYY-MM-DD")}`
